...
ORACLE_TRADES = Path(r"C:\Users\Notebook\Documents\Claude\Projects\Finanças\models\trades_all_with_pwin.csv")
...
class StrategyEngine:
    def __init__(self, model: FrozenModel, threshold: float, verbose: bool = False):
        ...
        self.stop_pct = model.strat["stop_pct"]
    ...
    def _decide(self, ts, o, h, l, c):
        ...
        self.armed = ArmedOrder(
            ...
            stop_price=extreme * (1 + self.stop_pct) if direction == -1 else extreme * (1 - self.stop_pct),
            mid=mid,
            opposite=opposite,
...
def compare_with_oracle(engine_trades: pd.DataFrame, model: FrozenModel, threshold: float):
    """Se ORACLE_TRADES existe, compara engine vs oracle trade-a-trade."""
    if not ORACLE_TRADES.exists():
        print(f"\n[regressao] oracle CSV nao existe em {ORACLE_TRADES}")
        print("  (rode freeze_2C_model.py apos eu adicionar save do CSV, ou pule esta etapa)")
        return
    print(f"\n[regressao] comparando engine vs oracle {ORACLE_TRADES.name}...")
    oracle = pd.read_csv(ORACLE_TRADES)
    ...
    if n_mismatch == 0 and len(only_oracle) == 0 and len(only_engine) == 0:
        print("  ✓ ENGINE BATE BYTE-A-BYTE COM ORACLE")
...
def main():
    ...
    if not MODEL_PATH.exists():
        print(f"[ERRO] modelo nao existe em {MODEL_PATH}")
        print("       rode primeiro: python scripts\\phase3\\freeze_2C_model.py")
        return 1

    model = FrozenModel(MODEL_PATH)
    thr = args.threshold if args.threshold is not None else model.threshold
    print(f"[engine 2C] modelo={model.d['model_version']} treino={model.d['train_date_utc']}")
    print(f"[engine 2C] threshold={thr}")

    trades = run_replay(model, thr)
    compare_with_oracle(trades, model, thr)
    return 0
