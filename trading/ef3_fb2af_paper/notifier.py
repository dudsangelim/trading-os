"""Telegram notifier for EF3 F-B.2.A.f — no-op when env vars not set."""
...
def notify_startup(equity: float) -> None:
    _send(
        ...
        f"*EF3 FB2Af PAPER ONLINE*\n"
        f"strategy: `{STRATEGY_ID}`\n"
        f"symbol: `{SYMBOL}` | tf: `H4`\n"
        f"capital: `${equity:.2f}`\n"
        f"mode: `1x no-leverage`\n"
        f"entry: 20\\-bar H4 breakout + compress <12% + SMA200\n"
        f"exit: SL 1xATR14 | TP 8:1 fixed"
    )
...
def notify_open(ts: str, entry_px: float, sl_price: float,
                tp_price: float, atr_signal: float, equity: float) -> None:
    _send(
