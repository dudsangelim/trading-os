"""
Freeze NY Open 2C v4 after the same-bar-retest look-ahead fix (2026-07-05).

v4 changes vs v3:
  - Labels use the corrected fill rule: the break bar can NOT be the retest
    (the model gate uses that bar's close, so the entry limit only exists after
    it closes). Retest search starts at the bar AFTER the break.
  - Paper leverage target raised to 6x, backed by the corrected OOS walk-forward
    (backtests/ny_open_2c_corrected_oos.py: +0.087%/trade, PF 1.67, p_boot=0.004;
    at 6x ~2.1%/mo backtest, hist MaxDD -16%, robust to edge-halving).
  - Threshold stays 0.60.
...
def collect_trades(df5m: pd.DataFrame, ctx: dict) -> pd.DataFrame:
    ...
    for date, g in iter_sessions(df5m):
        ...
        # v4 fix: retest can only start on the bar AFTER the break bar
        for i in range(break_pos + 1, end_search):
...
def main() -> int:
    ...
    trades_path = out_dir / "trades_v4_with_pwin.csv"
    model_path = out_dir / "2C_v4_frozen.json"
    ...
    model = {
        ...
        "execution_notes": {
            ...
            "rationale": (
                "v4 retrained after the same-bar-retest look-ahead fix: the break "
                "bar can no longer fill the entry (the gate uses its close). "
                "Corrected OOS walk-forward 2023-2026: +0.087%/trade, PF 1.67, "
                "p_boot=0.004, every year positive. Leverage target 6x: ~2.1%/mo "
                "backtest, hist MaxDD -16%, still positive if edge halves."
            ),
            "max_dd_alert_pct": 10,
            "max_dd_kill_pct": 18,
            "paper_only": True,
