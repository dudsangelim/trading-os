...
echo "DATABASE_URL: ${DATABASE_URL:-postgresql://jarvis:CHANGE_ME@postgres:5432/jarvis}"

export DATABASE_URL="${DATABASE_URL:-postgresql://jarvis:CHANGE_ME@postgres:5432/jarvis}"
...
async def go():
    pool = await asyncpg.create_pool(os.environ.get('DATABASE_URL', 'postgresql://jarvis:CHANGE_ME@postgres:5432/jarvis'), min_size=1, max_size=1)
