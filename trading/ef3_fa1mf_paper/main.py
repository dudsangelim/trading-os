"""
EF3 F-A.1.M.f paper trader — main entry point.
...
Health endpoint: GET /healthz on FA1_HEALTH_PORT (default 8098)
"""
...
from trading.ef3_fa1mf_paper.feed import fetch_bars, get_current_price
from trading.ef3_fa1mf_paper import notifier
...
class PaperTrader:
    def __init__(self) -> None:
        self.engine = Fa1mfEngine(initial_equity=INITIAL_CAPITAL)
        init_csvs()
        self._load()
        _health.update({
...
def _seconds_until_next_h4_trigger() -> float:
    """Return seconds until 2 minutes past the next H4 close (00/04/08/12/16/20 UTC)."""
...
def run_replay(trader: PaperTrader, days: int) -> None:
    ...
    trader.engine = Fa1mfEngine(initial_equity=INITIAL_CAPITAL)
    ...
    for ev in events:
        if ev["action"] == "open_long":
            log.info("  OPEN  long | %s | entry=%.4f stop=%.4f",
                     str(ev["ts"])[:19], ev["entry_px"], ev["trailing_stop"])
...
def main() -> int:
    ap = argparse.ArgumentParser(description="EF3 F-A.1.M.f paper trader")
    ...
    log.info(
        "EF3 FA1Mf v%s | strategy=%s symbol=%s capital=$%.0f",
        Fa1mfEngine.VERSION, STRATEGY_ID, SYMBOL, INITIAL_CAPITAL,
    )
